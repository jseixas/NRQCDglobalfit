...
//  	data->Branch("nColorChannels", &nColorChannels, "nColorChannels/I");
	...
	for(int iSampledPoint = 1; iSampledPoint <= nSampledPoints; iSampledPoint++){ // Sampling loop
		...
		dvector Op_state(NRQCDvars::nColorChannels,0);
		for (int i=0; i < nStates; i++){
			for (int j=0; j < NRQCDvars::nColorChannels; j++){
				//if(j==0) Op_state[j]=1; //Just to set it to some value for debugging purposes
				//if(j>0) Op_state[j]=0;//1-j*0.1; //Just to set it to some value for debugging purposes
				Op_state[j]=gRandom->Uniform(0,1);
			}
			Op[i]=Op_state;
		}
