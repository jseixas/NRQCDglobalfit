	 ...
	 if(MeasurementID==0){ // Modified by Joao
	  ...
	  for(int iRap = 0; iRap < nRapBins; iRap++){
		...
		if(nExp == NRQCDvars::LHCb || nExp == NRQCDvars::ALICE)
	      ...
	      else if(state == NRQCDvars::PSI_2S){
		...
		if(nExp == NRQCDvars::CDF)
	      ...
	      else if(state == NRQCDvars::UPS_1S || state == NRQCDvars::UPS_2S || state == NRQCDvars::UPS_3S){
		if(nExp == NRQCDvars::CMS) //normalize to the bin-width (pT and y)
	      ...
	      if(state == NRQCDvars::PSI_1S || state == NRQCDvars::PSI_2S || nExp == NRQCDvars::LHCb || nExp == NRQCDvars::ATLAS || nExp == NRQCDvars::CDF){//gives absolute uncertainties
		...
		//        relChangeNeg, relChangePos);
	      ...
	      else if((state == NRQCDvars::UPS_1S || state ==  NRQCDvars::UPS_2S || state == NRQCDvars::UPS_3S) && (nExp == NRQCDvars::CMS)){//gives relative uncertainties
...
//	    	if(iRap==0&&iP==0){
	    ...
	    sprintf(outname,"%s/ConvertedData_%s_%s_%s_rap%d_pT%d.txt",dirname, StateName[state],  MeasurementIDName[MeasurementID],  ExpName[nExp], iRap, iP);
...
//	    	}
	 ...
	 else printf("--->  Measurement not (yet) available\n");
