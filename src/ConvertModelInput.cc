...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// As long as we do not have access to Sergey's model, let's generate some toy data
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

		int n_nTuple=100000;

		TTree* nTupleModel[nStates][NRQCDvars::nColorChannels];
		char nTupleModelName[1000];

		double LuminosityPerModelEvent[nStates][NRQCDvars::nColorChannels];

		for (int iMother=0; iMother<nStates; iMother++){
			...
			for (int iColorChannel=0; iColorChannel<NRQCDvars::nColorChannels; iColorChannel++){
				...
				double weight; //nTupleModel[iMother][iColorChannel]->Branch("weight",      &weight,     "weight/D");
				Int_t decayClass; nTupleModel[iMother][iColorChannel]->Branch("decayClass",      &decayClass,     "decayClass/I");
				...
				weight=LuminosityPerModelEvent[iMother][iColorChannel];
				nTupleModel[iMother][iColorChannel]->SetWeight(weight);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		TTree* nTupleModel_Transformed[nStates][nStates][NRQCDvars::nColorChannels][NRQCDvars::nMaxCascades];//[iMother][iDaughter][iColorChannel]
		TGraph* DecayChainID[nStates][nStates][NRQCDvars::nColorChannels][NRQCDvars::nMaxCascades];//[iMother][iDaughter][iColorChannel]
			...
			for (int iDaughter=0; iDaughter<iMother; iDaughter++){
				...
				for (int iColorChannel=0; iColorChannel<NRQCDvars::nColorChannels; iColorChannel++){
					...
					for (int iCascade=0; iCascade<Buffer_DecayChains.size(); iCascade++){//Loop over all different decay chains linking iMother->iDaughter
						...
						Int_t decayClass_mother; nTupleModel[iMother][iColorChannel]->SetBranchAddress( "decayClass",  &decayClass_mother );
						...
						Int_t decayClass_daughter; nTupleModel_Transformed[iMother][iDaughter][iColorChannel][iCascade]->Branch("decayClass",      &decayClass_daughter,     "decayClass/I");
						...
						for ( int i_event = 1; i_event <= n_events; i_event++ ) {
							  ...
							  decayClass_daughter=0;

							  nTupleModel_Transformed[iMother][iDaughter][iColorChannel][iCascade]->Fill();

						}

						weight_daughter=nTupleModel[iMother][iColorChannel]->GetWeight()*Buffer_DecayChainBranchingRatios[iCascade]; // Luminosity/event times product of Branching ratios of the full decay chain
						nTupleModel_Transformed[iMother][iDaughter][iColorChannel][iCascade]->SetWeight(weight_daughter);
